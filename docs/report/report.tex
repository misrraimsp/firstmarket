...
\begin{document}
	...
	2020-07-18 17:20:19.718  INFO 4 --- [main] o.s.s.web.DefaultSecurityFilterChain: 
	Creating filter chain: 
	any request, [
	o.s.s.w.access.channel.ChannelProcessingFilter@5177e388, 
	o.s.s.w.context.request.async.WebAsyncManagerIntegrationFilter@69d6a7cd, 
	o.s.s.w.context.SecurityContextPersistenceFilter@73017a80, 
	o.s.s.w.header.HeaderWriterFilter@7c3ebc6b, 
	o.s.s.w.csrf.CsrfFilter@5bca7664, 
	o.s.s.w.authentication.logout.LogoutFilter@de18f63, 
	o.s.s.w.authentication.UsernamePasswordAuthenticationFilter@37b72ea, 
	o.s.s.w.savedrequest.RequestCacheAwareFilter@4a5905d9, 
	o.s.s.w.servletapi.SecurityContextHolderAwareRequestFilter@34a0ef00, 
	o.s.s.w.authentication.AnonymousAuthenticationFilter@3162743f, 
	o.s.s.w.session.SessionManagementFilter@6a9950f1, 
	o.s.s.w.access.ExceptionTranslationFilter@7f829c76, 
	o.s.s.w.access.intercept.FilterSecurityInterceptor@1dc2de84
	]
	\end{lstlisting}
	
	En estos filtros Spring Security implementa la autenticación/autorización y la protección frente a determinados tipos de ataques. Lo que resta es configurarlos para satisfacer las necesidades propias de cada aplicación. Esta configuración se lleva a cabo extendiendo la clase de Spring Security \emph{WebSecurityConfigurerAdapter} y sobreescribiendo su método \emph{configure(HttpSecurity)} (ver listado \ref{list:springsec_config}), tarea que en la aplicación web desarrollada realiza la clase \emph{SecurityConfig}.
	\\
	...
	\subsubsection{Protección ante Vulnerabilidades}
	...
	\begin{itemize}
		...
		Por ejemplo, si no se especifica política alguna que restrinja la información contenida en el encabezado \emph{Referer}, este podría contener el token de seguridad que FirstMarket adjunta en el link que envía a los usuarios para verificar la dirección de correo electrónico. La información sensible que puedan contener las URL quedaría expuesta en el encabezado \emph{Referer}.
		
		Para mitigar este riesgo es necesario, por tanto, especificar una política adecuada de \emph{Referer}, es decir, añadir un encabezado \emph{Referrer-Policy} con un valor apropiado. En este sentido, la política escogida para FirstMarket es \emph{strict-origin}. Esta política establece que sólo se envía información en el encabezado \emph{Referer} si el destinatario usa HTTPS. Esta es la parte \emph{strict} de \emph{strict-origin}. La parte \emph{origin} significa que sólo se envía el origen de la dirección. Por ejemplo, de www.firstmarket.tech/login?error=true sólo sería enviado www.firstmarket.tech.
		
		En el listado \ref{list:referrer_policy} se detalla el fragmento del método \emph{configure}, de la clase \emph{SecurityConfig}, donde se configura el uso de esta política.
		\\
		
		\begin{lstlisting}[language=Java,caption=Configuración de \emph{Referrer-Policy} como \emph{strict-origin},label=list:referrer_policy]
		@Override
		protected void configure(HttpSecurity httpSecurity) throws Exception {
			httpSecurity
				//...
