...
\begin{document}
   ...
   This Final Degree Project presents a book e-commerce web application, named \href{https://firstmarket.tech}{FirstMarket}. The goal has been to delve into the problems faced by these technologies and the modern ways of solving them, always keeping in mind the tools seen in the course of the study plan.
   ...
   \clearpage
    ...
    Una \textbf{aplicación web} es el sistema software encargado de la generación dinámica de páginas web. Normalmente, y a diferencia del modelo estático, entre sus componentes se encuentra una base de datos que permita la persistencia de la información, cambiante por definición de la propia arquitectura.
    ...
    En la sección \ref{sec:budget} se ofrece un presupuesto estimativo de todos los costes asociados con el desarrollo del presente proyecto.
    
    Por último, para concluir con esta memoria, en la sección \ref{sec:improvements} se comentan determinados posibles campos de mejora y ampliación de la aplicación desarrollada.
    
    \chapter{Análisis} \label{sec:analysis}
    En esta sección se aborda la fase de análisis del desarrollo software. Partiendo de la especificación de los requisitos básicos que debe implementar la aplicación web, se ofrece un conjunto de casos de uso que dé una visión de cuál debe ser el flujo de interacción.
        ...
        \section{Requisitos}
            ...
            \begin{enumerate}
                ...
                \begin{itemize}
                    ...
                    \item[-] (deseable) Un UA debe poder consultar las promociones disponibles.
                ...
                \begin{itemize}
                    ...
                    \item[-] (deseable) Puntuar (de alguna manera, p.e. estrellas del 1 al 5) un determinado libro que haya adquirido. Debe poder hacerlo en cualquier momento tras la compra.
                    ...
                    \item[-] Cerrar sesión.
                    \item[-] Un UR debe poder ponerse en contacto con el administrador de la aplicación web, por ejemplo a través de una dirección de correo eletrónico.
                \end{itemize}
                ...
                \begin{itemize}
                    ...
                    \item[-] (deseable) Crear, editar o eliminar promociones de libros (CRUD promociones).
                    ...
                    \item[-] (deseable) Poder generar informes (p.e. ventas durante un determinado periodo con su importe y la facturación total).
                \end{itemize}
            \end{enumerate}
    ...
    Los proyectos Spring usados en el desarrollo de la aplicación web han sido, siguiendo la línea básica descrita en \cite{Walls2019}, \emph{Spring Framework}, \emph{Spring Boot}, \emph{Spring Data JPA} y \emph{Spring Security}. En la presente sección se dará una descripción de las características de cada uno de ellos relacionadas con el desarrollo del presente proyecto, salvo aquellas referentes a Spring Security, tratadas por separado en la sección \ref{sec:springsec} en el marco de la seguridad de la aplicación web.
    
    \paragraph{Spring Framework} 
    Esta es la parte fundamental dentro del ecosistema Spring. Tomando como principal fuente su propia documentación (\cite{Johnson2020}), los principios de diseño que han guiado su desarrollo a lo largo del tiempo han sido:
    ...
    Los comercios en Internet tienen, además, la responsabilidad de ofrecer altos estándares de seguridad a sus clientes, especialmente en lo concerniente con el pago. De hecho, una de las principales barreras que aparta a potenciales usuarios del comercio \emph{online} es la percepción de inseguridad que el proceso de pago telemático lleva asociado. En este sentido, el comercio a través de Internet depende radicalmente de la confianza de los usuarios, siendo así este uno de los principales capitales a cultivar y fortalecer.
    ...
    Notar en la figura \ref{fig:stripe} que cuando se produce el \emph{timeout} y el pago no se ha efectuado se activa la vía de cancelar el PaymentIntent (como se verá en la sección \ref{sec:concurrencia}, descomprometiendo la cesta).
    ...
    \begin{itemize}
    	...
    	\item[-] \emph{payment\_intent.amount\_capturable\_updated}. Tiene lugar cuando el PaymentIntent posee fondos aún por capturar. No se realiza ninguna acción aparte de su registro.
    \end{itemize}
    ...
    \begin{itemize}
    	...
    	\item[-] Se contacta con Stripe para crear un nuevo \emph{PaymentIntent}.
    ...
    \begin{itemize}
    	...
    	\item[-] Se contacta con Stripe para cancelar el \emph{PaymentIntent}.
    \end{itemize}
	...
	Además, Spring Security permite especificar la lógica tras un \emph{logout}, que en el caso de la presente aplicación es simplemente una redirección a la página de inicio, como se muestra en las líneas 11 y 12 del listado \ref{list:springsec_config}.
	...
	Como se puede apreciar en el listado \ref{list:springsec_config}, líneas de la 14 a la 25, FirstMarket implementa tres tipos de recursos: 
	
	\begin{itemize}
		\item[-] Los que sólo pueden ser accedidos por usuarios con rol \emph{ROLE\_ADMIN}, ubicados en \emph{\nolinkurl{~/admin/**}}.
		\item[-] Los que sólo pueden ser accedidos por usuarios con rol \emph{ROLE\_USER}, ubicados en \emph{\nolinkurl{~/user/**}}.
	...
	Muchos navegadores incluyen filtros anti-\href{https://owasp.org/www-community/attacks/xss/}{XSS}. Aún no representando una solución infalible al problema, ayudan mucho a su mitigación. Normalmente estos filtros están habilitados por defecto, siendo posible configurar qué hacer en caso de un positivo (por ejemplo, intentar enmendar el contenido detectado).
	...
	Spring Security permite de una manera sencilla configurar la política deseada \cite{Alex2020}, que en el caso de FirstMarket ha sido \textbf{\emph{strict-origin}}, como se aprecia en las líneas 81 y 84 del listado \ref{list:springsec_config}. Esta política establece que sólo se envía información en el encabezado \emph{Referer} si el destinatario usa HTTPS. Esta es la parte \emph{strict} de \emph{strict-origin}. La parte \emph{origin} significa que sólo se envía el origen de la dirección. Por ejemplo, de \emph{www.firstmarket.tech/login?error=true} únicamente sería incluido en el encabezado \emph{www.firstmarket.tech}, ocultando así el resto de detalles.
    ...
    \chapter{Mejoras y Ampliaciones} \label{sec:improvements}
    Tras la finalización del presente proyecto se han detectado diversos puntos de potencial mejora, pertenecientes a diferentes ámbitos. A continuación se comenta cada uno de ellos:
    
    \begin{itemize}
    	\item[-] Una de las principales (si no la principal) ampliación que se le puede (debe) aplicar al presente proyecto es el desarrollo de pruebas automatizadas que cubran la mayor cantidad de funcionalidades posible. Este es un punto crítico para el mantenimiento y/o ampliación de funcionalidades futuras.
    	
    	Tal como está en la actualidad resulta ser un sistema software muy frágil en el sentido de los errores que pueda contener, y de los que se pueda introducir con cualquier actualización o ampliación de funcionalidad.
    	
    	En esta línea, el desarrollo de test con herramientas como JUnit, Mockito, AssertJ, etc. son muy necesarias para garantizar la calidad del producto y mejorar la experiencia de desarrollo y depuración.
    	
    	\item[-] Sería muy interesante implementar una funcionalidad que permita a los usuarios emitir valoraciones de los libros que han adquirido, por ejemplo con un sistema de estrellas de 1 a 5. Esto permitiría, entre otras cosas, obtener información de los gustos de cada usuario y mejorar en su segmentación.
    	
    	\item[-] Otra ampliación deseable, que en realidad formaba parte del esbozo de requisitos deseables, pero que por tiempo no se implementó, sería habilitar un sistema de ofertas. De esta forma el administrador de la tienda podría establecer descuentos o cupones sobre determinados productos, con posiblemente determinadas ventanas temporales de validez, etc.
    	
    	\item[-] El sistema de búsqueda de artículos en una tienda online es vital. El usuario debe notar que es fluida, sencilla, eficiente, amigable, intuitiva, y que, sobretodo, le permita encontrar lo que busca rápidamente. En este sentido, aunque el sistema de búsqueda desarrollado cumple con su cometido básico, es muy mejorable. Una posible alternativa sería externalizar esta funcionalidad con servicios como el que ofrece \href{https://www.algolia.com/}{Algolia}.
    	
    	\item[-] Profundizar en la \href{https://stripe.com/docs/radar/checklist}{integración} con Stripe, de forma que le facilitemos información de los clientes, utilizando la \href{https://stripe.com/docs/api/customers}{Customer API}. Esto le permite a la plataforma de pagos mejorar considerablemente el éxito en la predicción de fraudes en los pagos, ya que enriquece el modelo de machine learning subyacente.
    	
 
    	\item[-] En el plano de la seguridad, aunque se ha realizado un gran esfuerzo, cabría realizar pruebas de pentesting, con herramientas como por ejemplo \href{https://www.zaproxy.org/}{OWASP Zed Attack Proxy}.
