...
\begin{document}
    ...
    %Centrando la atención ahora en el núcleo de la aplicación, es decir, dejando de lado los servicios periféricos de pago y correo, los principios de diseño aplicados más relevantes han sido la \textbf{inyección de dependencias} (\emph{dependency injection}, DI) y la \textbf{separación de responsabilidades} (\emph{separation of concerns}, SoC). El primero se obtiene de la mano del contenedor de inversión de control usado para el desarrollo de la aplicación -Spring Framework [ref]-, mientras que al segundo se llega organizando funcionalmente la aplicación en capas: web, negocio y persistencia. A continuación se comentará cada uno de estos principios.
    
    
    \subsubsection{Inversión de Control. Inyección de Dependencias}
    %La inversión de control es un concepto de gestión de la complejidad que hoy en día prácticamente cualquier aplicación Java Empresarial (y fuera del mundo Java también) utiliza.
    ...
    Un mecanismo efectivo para separar la construcción del uso es la \textbf{inyección de dependencias} (\emph{dependency injection}, DI), que, tal como explica Martin Fowler en su artículo seminal en la materia [bib ref], es un tipo de \textbf{inversión de control} (\emph{inversion of control}, IoC) para la gestión de dependencias. Por inversión de control se entiende la técnica de traspasar las responsabilidades secundarias de un objeto a otros objetos especializados para el propósito. Así, en el contexto de gestión de dependencias, un objeto no debe asumir la responsabilidad de instanciar él mismo sus dependencias. En su lugar, debe ceder esta responsabilidad a otra entidad especializada, invirtiendo así el control. Debido a que la configuración inicial es una cuestión global, esta entidad especializada será la rutina \emph{main} o un contenedor especializado en este propósito. En el caso de la presente aplicación web se ha usado el contenedor de inyección de dependencias del mundo Java líder en la actualidad, Spring Framework.
    
    
    \subsubsection{Separación de Responsabilidades}
    libro de martin fowler, pag 19
    
    \begin{figure}[htb!]
    	\centering
    	\includegraphics[width=0.75\textwidth,keepaspectratio]{desing_layer_diagram}
    \end{figure}
    ...
    \begin{figure}[htb!]
    	...
    	\caption{Modelo de Dominio}  \label{fig:dominio}
    \end{figure}
    ...
    roll back. Partial completion is not a transactional concept. Thus, if Mar-
    tin is transferring some money from his savings to his checking account
    and the server crashes after he’s withdrawn the money from his savings,
    the system behaves as if he never did the withdrawal. Committing says
    both things occurred; a roll back says neither occurred. It has to be both
    or neither.
    ...
    -Isolation: The result of an individual transaction must not be visible to any
    other open transactions until that transaction commits successfully.
    -Durability: Any result of a committed transaction must be made perma-
    nent. This translates to “Must survive a crash of any sort.”
    \\
