...
public class SearchController {
    ...
    @Autowired
    public SearchController(FilterServer filterServer, CatServer catServer, BookServer bookServer) {
    ...
    public String showNewSearch(@PathVariable("categoryId") Long categoryId, Model model){
        ...
        Filter filter = filterServer.create(category);
        List<Book> books = bookServer.findWithFilter(filter);

        List<Author> authors = bookServer.findTopAuthors(books);
        List<Publisher> publishers = bookServer.findTopPublishers(books);
        List<Languages> languages = bookServer.findTopLanguages(books);
