...
public class StringToOrderSortCriteriaConverter implements Converter<String, OrderSortCriteria> {
    ...
    public OrderSortCriteria convert(String s) {
        ...
        try {
            ...
            if (index > maxIndex) {
                LOGGER.warn("OrderSortCriteria index({}) out-of-bound({}). Defaults to {}", index, maxIndex, OrderSortCriteria.DATE_DESC);
                return OrderSortCriteria.DATE_DESC;
            }
            return OrderSortCriteria.values()[index];
        }
        catch (NumberFormatException e) {
            LOGGER.error("OrderSortCriteria index string({}) can not be converted due to format exception. Defaults to {}", s, OrderSortCriteria.DATE_DESC, e);
            return OrderSortCriteria.DATE_DESC;
        }
    }
}
